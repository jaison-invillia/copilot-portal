...
- Writing tests
- Delegating execution to other agents
...
### Step 6 — Verify whether documentation/testing requests are already planned
Before posting the final plan, verify if this issue already includes BOTH:
- A relevant subtask checklist item in the issue body
- A prior agent comment indicating the same guidance has already been requested/provided

If BOTH are present, mark the requirement as already planned.
If one or both are missing, include explicit requests in your architectural comment for:
- `documenter`: how to document the demand
- `test-advisor`: how to test the demand at a high level

Important: requests must be written in the issue comment. Do not delegate execution.

### Step 7 — Post architectural plan as a comment (MCP)
...
### Documentation & high-level testing coverage check

- **Subtask in issue body found?** ✅ / ❌
- **Prior agent comment found?** ✅ / ❌
- **Already planned (both true)?** ✅ / ❌
...
- Never delegate implementation or execution to other agents. Your role is architectural analysis and issue guidance only.
